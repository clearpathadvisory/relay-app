...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const fixed: MetadataRoute.Sitemap = [
    ...
    { url: base + '/vs-stan', lastModified: now, changeFrequency: 'monthly', priority: 0.8 },
    { url: base + '/blog', lastModified: now, changeFrequency: 'weekly', priority: 0.8 },
    { url: base + '/privacy', lastModified: now, changeFrequency: 'yearly', priority: 0.3 },
    { url: base + '/terms', lastModified: now, changeFrequency: 'yearly', priority: 0.3 },
  ]

  // Posts first: they are the pages we most want crawled, and a failure to
  // read them must not take the profile half of the sitemap down with it.
  ...
  try {
    ...
    const rows = data || []
    if (!rows.length) return fixed.concat(posts)
    ...
    return fixed.concat(posts).concat(pages)
  } catch (e) {
