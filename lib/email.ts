...
const FROM = process.env.RESEND_FROM || 'RelayMe <hello@relayme.bio>'
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border:1px solid #EFEAF7;border-radius:22px;overflow:hidden;">
        <tr><td style="padding:26px 30px 0;">
          <span style="font:800 21px/1 -apple-system,'Segoe UI',Manrope,Arial,sans-serif;color:#1B0D44;letter-spacing:-0.5px;">RelayMe</span>
        </td></tr>
        ...
        <tr><td style="padding:18px 30px 24px;border-top:1px solid #F1EDF8;font:400 12.5px/1.6 -apple-system,'Segoe UI',Manrope,Arial,sans-serif;color:#6B5FA8;">
          ${footer || 'RelayMe is made by ClearPath Advisory. You are getting this because you have a RelayMe account.'}
        </td></tr>
      </table>
...
export function accountClosedEmail(username: string) {
  return shell(
    'Your RelayMe account is closed',
    `<p style="margin:0 0 14px;">Your account has been deleted, and so has everything in it: your page at
     ...
     <p style="margin:0;">Thanks for trying RelayMe. If something drove you away, we would genuinely like to
...
export function subscriptionCancelledEmail(endsOn: string | null) {
  ...
  return shell(
    ...
    `<p style="margin:0 0 14px;">We have cancelled your RelayMe Pro subscription, so it will not renew.
      ${until}</p>
...
export function subscriptionEndedEmail() {
  return shell(
    ...
    `<p style="margin:0 0 14px;">Your RelayMe Pro period has finished and your account is back on the free plan.</p>
...
export function confirmSubscriptionEmail(pageName: string, pageUrl: string, confirmUrl: string) {
  return shell(
    ...
    'RelayMe sent this on behalf of ' + pageName + '. RelayMe does not email you itself, and you can leave at any time from any message they send.'
  )
}
...
export function paymentFailedEmail(willRetry: boolean) {
  return shell(
    ...
    `<p style="margin:0 0 14px;">Your card was declined, so this month&rsquo;s RelayMe payment has not gone through.</p>
...
export function welcomeEmail(username: string) {
  ...
  return shell(
    ...
    'You are getting this because you claimed a name on RelayMe. Reply to this message if anything is unclear — a person reads it.'
  )
}

export function upgradedEmail(interval: 'month' | 'year', renewsOn: string | null) {
  return shell(
    'You are on RelayMe Pro',
    `<p style="margin:0 0 14px;">Payment received, and everything is unlocked: all 47 themes, all 8 fonts,
      ...
      the page, email capture, and the RelayMe badge gone if you want it gone.</p>
    ...
    'RelayMe is made by ClearPath Advisory, NIP 9512604332, Warsaw, Poland.'
  )
}
